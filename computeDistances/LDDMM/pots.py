...
def testpot():
    include_shoot=True
    scale=0.5
    Qr = get_data('data/pot1.txt')
    Qt = get_data('data/pot3.txt')
...
def run_quick():
    ...
    dict['lam']=0.0
    dict['beta']=0
    dict['fname']="figs/morepots/pot_"
    noise_vars=np.array([0.0])
    exts=['xa']
    dict['ext']=exts[0]
    listing = glob.glob('data/morepots/pot_*')
    dists = np.zeros((len(listing),len(listing)))
    f = open('quickOrder2.txt','w')
    f.write(str(listing))
    f.close()
    ...
    np.save('quick_dists_reg2.txt',dists)
